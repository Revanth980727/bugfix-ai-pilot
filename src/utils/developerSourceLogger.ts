...
export function extractGitHubSourceFromEnv(): GitHubSource {
  ...
  return {
    ...
    default_branch: process.env.GITHUB_DEFAULT_BRANCH || import.meta.env.VITE_GITHUB_DEFAULT_BRANCH as string,
...
export function logGitHubSource(source: GitHubSource | null): void {
  ...
  console.log(`Branch: ${source.branch || source.default_branch}`);
  console.log(`Patch Mode: ${source.patch_mode}`);
